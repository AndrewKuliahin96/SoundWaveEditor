    ...
    override fun readFile(file: File) {
        ...
        while (pos < fileSizeBytes - 12) {
            ...
            if (progressListener != null) {
                if (progressListener?.reportProgress(pos * 1.0 / fileSizeBytes) != true) {
                    break
                }
            }
            ...
            val bitRate = when (mpgVersion) {
                1 -> BIT_RATES_MPEG1_L3[buffer.getInt(2) and 0xF0 shr 4]
                else -> BIT_RATES_MPEG2_L3[buffer.getInt(2) and 0xF0 shr 4]
            }

            val sampleRate = when (mpgVersion) {
                1 -> SAMPLE_RATES_MPEG1_L3[buffer.getInt(2) and 0x0C shr 2]
                else -> SAMPLE_RATES_MPEG2_L3[buffer.getInt(2) and 0x0C shr 2]
            }
